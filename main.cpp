...
int main(int argc, char* argv[])
{
    ...
    {
        std::string full_config_path = std::string("../../") + argv[1];
        sim.LoadConfig(full_config_path);  
    }
    sim.Run();
}
...
//     auto liftingScheme = std::make_shared<Prandtl::LiftingBR1>();
...
//     lid = 0; lid[2] = 1;
//     NS.AddBdrFaceIntegrator(new Prandtl::NoSlipAdiabWallBdrFaceIntegrator(liftingScheme, numericalFlux, order + 1, NS.GetTimeRef(), qn, vn), walls);
//     NS.AddBdrFaceIntegrator(new Prandtl::NoSlipAdiabWallBdrFaceIntegrator(liftingScheme, numericalFlux, order + 1, NS.GetTimeRef(), qn, vlid), lid);
