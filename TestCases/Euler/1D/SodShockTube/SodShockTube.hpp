...
std::function<void(const Vector&, Vector&)> SodShockTubeIC(real_t gamma)
{
...
struct RegisterSodShockTube
{
    RegisterSodShockTube()
    {
        ...
        ConditionFactory::Instance().RegisterInitialCondition1("SodShockTubeIC", SodShockTubeIC);
        
        // Register boundary condition for left and right boundaries.
        ConditionFactory::Instance().RegisterVectorFunctionBoundaryCondition1("SodShockTubeLeftBC", SodShockTubeIC);
        ConditionFactory::Instance().RegisterVectorFunctionBoundaryCondition1("SodShockTubeRightBC", SodShockTubeIC);
    }
};
